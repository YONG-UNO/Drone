    ...
    for (;;) {
        ...
        if (friction_wheel_enabled == 1) { // 当摩擦轮启动时才可以开启拨弹盘
            if (RC.dialWheel >= 1024 + 440) {
                input_2006 = pidSpeed(&speed_pid_CAN_2006_M3_ID, target_rpm_M3, motor_measure[2].speed_rpm);
            }else if (RC.dialWheel < 1024+440 && RC.dialWheel >= 700) {
                input_2006 = 0;
                pidSpeed(&speed_pid_CAN_2006_M3_ID, 0, motor_measure[2].speed_rpm);
